...
export default function Home() {
    ...
    const dashboardData = useMemo(() => {
        ...
        //자산 계산
        const totalAssets = transactions.filter((t) => t.accountType === "asset").reduce((sum, t) => (t.type === "income" ? sum + t.amount : sum - t.amount), 0);

        const totalLiabilities = transactions.filter((t) => t.accountType === "liability").reduce((sum, t) => (t.type === "expense" ? sum + t.amount : sum - t.amount), 0);
        ...
        const expenseByCategory = monthlyTransactions
            ...
            .reduce((acc, t) => {
                acc[t.leftAccountName] = (acc[t.leftAccountName] || 0) + t.amount;
                return acc;
                        ...
                        : "이체"}
                            ...
                            {recentTransactions.map((transaction, index) => (
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        <p className={`font-bold text-sm ${getTransactionColor(transaction.type)}`}>{transaction.amount.toLocaleString()}원</p>
                                        <Badge
                                            variant="outline"
                                            className={`text-xs ${
                                                transaction.type === "income"
