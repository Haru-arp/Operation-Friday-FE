...
import { CreditCard, TrendingUp, RefreshCw, HandCoins, DollarSign, Calendar, Check, Info, TrendingDown } from "lucide-react";
...
const settingTransactionTypes = {
    debt_transfer: {
        ...
        credit: "부채+",
        category: "부채 이동",
        guide: "기존에 있던 부채를 새로운 부채로 바꾸는 거래입니다. 예를 들어 신용카드 대금을 대환대출로 갚거나, 카드 리볼빙을 이용하는 경우입니다.",
    },
    asset_initial: {
        id: "asset_initial",
        name: "자산 초기 등록",
        ...
        credit: "자본+",
        category: "초기 세팅",
        guide: "가계부를 시작할 때 현재 보유하고 있는 자산을 등록하는 거래입니다. 현금, 예금, 투자자산 등을 초기 자본으로 설정합니다.",
    },
    debt_asset_creation: {
        ...
        credit: "부채+",
        category: "초기 세팅",
        guide: "대출을 받아서 자산이 생긴 경우입니다. 전세보증금 대출, 학자금 대출 등으로 계좌에 돈이 들어온 상황을 기록합니다.",
    },
    asset_decrease: {
        id: "asset_decrease",
        name: "자산 감소 (자본 회수)",
        ...
        credit: "자산-",
        category: "자본 회수",
        guide: "개인 자본에서 자산을 빼내는 거래입니다. 투자금을 회수하거나 장부 오류를 정정할 때 사용합니다.",
    },
    debt_only_start: {
        id: "debt_only_start",
        name: "부채만 있는 시작",
        ...
        credit: "부채+",
        category: "초기 세팅",
        guide: "가계부를 시작할 때 자산은 없고 부채만 있는 상황을 기록합니다. 신용카드 대금이나 대출만 있는 상태에서 시작하는 경우입니다.",
    },
    capital_reclassify: {
        ...
        credit: "자본+",
        category: "자본 이동",
        guide: "자본의 성격을 바꾸는 거래입니다. 개인 자본을 사업 자금으로 전환하거나, 가족 지원금을 개인 자산으로 전환하는 경우입니다.",
    },
};
export const InitialTransactionWizard = ({ onSave, transactionType }: InitialTransactionWizardProps) => {
    ...
    const selectedType = settingTransactionTypes[transactionType as keyof typeof settingTransactionTypes];
    ...
    const getFilteredAccounts = (position: "debit" | "credit") => {
        ...
        const relevantGroupIds = accountData.groups.filter((group) => group.type === targetAccountType).map((group) => group.id);
        ...
        <Card className="w-full">
            <CardHeader>
                <CardTitle className="flex items-center gap-3">
                    <div className={`w-10 h-10 ${selectedType.color} rounded-lg flex items-center justify-center`}>
                        <Icon className="h-5 w-5 text-white" />
                    </div>
                    <div>
                        <div>{selectedType.name}</div>
                        <div className="text-sm font-normal text-gray-500">{selectedType.category}</div>
                    </div>
                </CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                    <h3 className="font-medium text-blue-900 dark:text-blue-100 mb-2">이 거래는 언제 사용하나요?</h3>
                    <p className="text-sm text-blue-800 dark:text-blue-200 mb-3">{selectedType.guide}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="text-center">
                            ...
                            <div className="bg-white dark:bg-gray-700 p-3 rounded-lg border-2 border-blue-200 dark:border-blue-700">
                                <div className="font-medium text-blue-900 dark:text-blue-100 text-sm">{selectedType.debit}</div>
                                <div className="flex items-center justify-center mt-1">
                                    {selectedType.debit.includes("+") ? <Plus className="h-4 w-4 text-green-600" /> : <Minus className="h-4 w-4 text-red-600" />}
                                    <span className="text-xs text-gray-600 dark:text-gray-400 ml-1">{selectedType.debit.includes("+") ? "증가" : "감소"}</span>
                                </div>
                            </div>
                        </div>
                        <div className="text-center">
                            ...
                            <div className="bg-white dark:bg-gray-700 p-3 rounded-lg border-2 border-green-200 dark:border-green-700">
                                <div className="font-medium text-green-900 dark:text-green-100 text-sm">{selectedType.credit}</div>
                                <div className="flex items-center justify-center mt-1">
                                    {selectedType.credit.includes("+") ? <Plus className="h-4 w-4 text-green-600" /> : <Minus className="h-4 w-4 text-red-600" />}
                                    <span className="text-xs text-gray-600 dark:text-gray-400 ml-1">{selectedType.credit.includes("+") ? "증가" : "감소"}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                        ...
                        <Select value={debitAccount} onValueChange={setDebitAccount}>
                            ...
                            <SelectContent>
                                {getFilteredAccounts("debit").map((account: AccountWithGroupPath) => (
                                    <SelectItem key={account.id} value={account.id}>
                                        <div className="flex flex-col items-start">
                                            <span className="font-medium">{account.name}</span>
                                            <span className="text-xs text-gray-500">{account.groupPath}</span>
                                        </div>
                                    </SelectItem>
                                ))}
                            </SelectContent>
                    ...
                    <div className="space-y-2">
                        ...
                        <Select value={creditAccount} onValueChange={setCreditAccount}>
                            ...
                            <SelectContent>
                                {getFilteredAccounts("credit").map((account: AccountWithGroupPath) => (
                                    <SelectItem key={account.id} value={account.id}>
                                        <div className="flex flex-col items-start">
                                            <span className="font-medium">{account.name}</span>
                                            <span className="text-xs text-gray-500">{account.groupPath}</span>
